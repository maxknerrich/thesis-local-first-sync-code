...
import { createAndUpdate5IssuesParallel, createAndUpdate25IssuesParallel, resetRepoToNormalState } from "../scripts/test-scenarios";
import {
	APPS,
	RUNS_PER_TEST,
	getNetworkProfile
...
const manager = new ResultsManager('UC5', {
...
test.afterEach(async () => {
	console.log(`Resetting repository for issue count: ${issue_count}`);
	await resetRepoToNormalState(`thesis-test-dynamic-${issue_count}`, issue_count);
});


test.describe('UC5 - Local First', () => {
	const ttvfSeries = new TimeSeriesLogger("UC5", `ttvf-${syncInterval}`, "", "local-first", ["issueCount", "syncInterval", "run", "createTTVF", "updateTTVF"]);
	for (let i = 1; i <= RUNS_PER_TEST; i++) {
		test(`LF - UC 3 - RUN ${i} - ISSUE_COUNT ${issue_count}`, async ({
			page,
			context,
		}) => {
			const timeSeries = new TimeSeriesLogger("UC5", i, issue_count, `local-first${syncInterval}`);
			...
			for (let j = 5 * 4; j >= 1; j--) {
				...
				await page.getByRole("link", { name: `Issue ${issue_count}/${issue_count} for thesis-test-dynamic-${issue_count}` }).click();
				await page.getByRole('textbox', { name: 'Issue Title' }).click();
				const currentTitle = await page.getByRole('textbox', { name: 'Issue Title' }).inputValue();
				// Extract original title by removing any "LOCAL UPDATE X " prefix
				const originalTitle = currentTitle.replace(/^LOCAL UPDATE \d+ /, '');
				await page.getByRole('textbox', { name: 'Issue Title' }).fill(`LOCAL UPDATE ${j} ${originalTitle}`);
				await page.getByRole('button', { name: 'Save & Back' }).click();
				const updateStartTime = performance.now();
				await page.getByText(`LOCAL UPDATE ${j} ${originalTitle}`).waitFor({ state: "visible" });
				const updateEndTime = performance.now();


				ttvfSeries.logTTVF(0, updateEndTime - updateStartTime, i, issue_count, syncInterval);

				// Wait for remaining time to reach 15 seconds total per operation
				if (j > 1) {
					const operationDuration = Date.now() - operationStartTime;
					const remainingDelay = Math.max(0, 15000 - operationDuration);
					if (remainingDelay > 0) {
...
test.describe('UC5 - Cloud', () => {
	const ttvfSeries = new TimeSeriesLogger("UC5", "ttvf", "", "cloud", ["issueCount", "syncInterval", "run", "createTTVF", "updateTTVF"]);
	...
	for (let i = 1; i <= RUNS_PER_TEST; i++) {
		test(`CLOUD - UC 3 - RUN ${i} - ISSUE_COUNT ${issue_count}`, async ({
			...
			const timeSeries = new TimeSeriesLogger("UC5", i, issue_count, "cloud");
			const client = await context.newCDPSession(page);
			...
			for (let j = 5 * 4; j >= 1; j--) {
				const operationStartTime = Date.now();
				await page.goto(`http://localhost:4173/maxknerrich/thesis-test-dynamic-${issue_count}/issue/${issue_count}`)
				await page.waitForLoadState("networkidle");

				const currentTitle = await page.getByRole('textbox', { name: 'Title' }).inputValue();
				// Extract original title by removing any "LOCAL UPDATE X " prefix
				const originalTitle = currentTitle.replace(/^LOCAL UPDATE \d+ /, '');
				await page.getByRole('textbox', { name: 'Title' }).fill(`LOCAL UPDATE ${j} ${originalTitle}`);
				await page.getByRole('button', { name: 'Save Changes' }).click();
				const updateStartTime = performance.now();
				await page.getByText(`LOCAL UPDATE ${j} ${originalTitle}`).waitFor({ state: "visible" });
				const updateEndTime = performance.now();

				ttvfSeries.logTTVF(0, updateEndTime - updateStartTime, i, issue_count, "none");
